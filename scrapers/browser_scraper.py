...
Works for: IEEE Xplore (authors section), some ACM pages,
           any conference with JS-rendered author blocks.
...
BROWSER_DOMAINS = [
    "ieeexplore.ieee.org",
    "dl.acm.org",           # fallback if HTML tier fails
    "link.springer.com",
    "nature.com",
]

# Per-domain interaction rules: before reading the DOM, do these actions.
# Each entry is a list of (action, selector_or_value) tuples.
# Actions: "click", "wait", "scroll"
DOMAIN_INTERACTIONS = {
    "ieeexplore.ieee.org": [
        ("click", "text=Authors"),          # expand Authors accordion
        ("wait", "1000"),                   # ms
    ],
    "dl.acm.org": [
        ("scroll", "500"),                  # px down to load lazy content
        ("wait", "500"),
    ],
}

# Paywall signals — if any appear in page text, bail immediately
PAYWALL_SIGNALS = [
...
class BrowserScraper(BaseScraper):
    ...
    def scrape(self, url: str) -> ScrapeResult:
        try:
            # Import here so missing playwright doesn't break the whole pipeline
